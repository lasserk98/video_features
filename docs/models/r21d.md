...
To load your own weights, set `checkpoint_path` to a `.pth/.pt/.ckpt`; Lightning checkpoints are auto-converted unless you set `auto_convert_checkpoint=false`. You can also pre-clean a checkpoint:
```bash
python tools/convert_checkpoint.py --in your.ckpt --out cleaned_r21d.pth --model generic
```
...
| `checkpoint_path`                       | `null`                                | Path to a custom R(2+1)D checkpoint (`.pth/.pt/.ckpt`). If set, the model is loaded from this path instead of the default weights.                                                 |
